        ...
        if r.boxes and r.boxes.id is not None:
            ...
            for id, box in zip(ids, boxes):
                ...
                if area in [8, 9, 10, 18]:
                    ...
                    if not os.path.isdir(os.path.join(save_dir, "person" + str(id), "crops")):
                        os.makedirs(os.path.join(save_dir, "person" + str(id), "crops"), exist_ok=True)
                        
                    if area in [9, 10, 18] and (x1 + x2)/2 < 1280 / 2:
                        save_txt_path = os.path.join(save_dir, "person" + str(id), "track.txt")
                        ...
                        cv2.imwrite(os.path.join(save_dir, "person" + str(id), "crops", "{}.jpg".format(frame_id)), image_crop)
                        
                    elif area in [8, 9] and (x1 + x2)/2 >= 1280 / 2:
                        save_txt_path = os.path.join(save_dir, "person" + str(id), "track.txt")
                        if not os.path.isfile(save_txt_path):
                            with open(save_txt_path, 'w') as f:
                                f.write(f'{frame_id+1} {id} {area} {x1} {y1} {x2-x1} {y2-y1}\n')
